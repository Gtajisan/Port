/**
 * Goat.ig.js — Instagram-mode entry point for Baka-Chan Bot
 *
 * Identical to Goat.js in every way, EXCEPT:
 *  - loads .env first
 *  - always requires bot/login/loginInstagram.js
 *
 * Spawned by index.js when INSTAGRAM_MODE=true.
 */
...
// Node_ENV is not set in IG mode — always use non-dev files
...
// Config watchers (same as Goat.js)
...
(async () => {
  global.utils.sendMail = async () => { throw new Error("Email functionality has been removed from Baka-Chan Bot"); };
  global.utils.transporter = null;

  // Version check (non-blocking)
  try {
    const { data: { version } } = await axios.get("https://raw.githubusercontent.com/Gtajisan/baka-chan-bot/main/package.json");
    const currentVersion = require("./package.json").version;
    if (compareVersion(version, currentVersion) === 1)
      utils.log.master("NEW VERSION", getText(
        "Goat", "newVersionDetected",
        colors.gray(currentVersion),
        colors.hex("#eb6a07", version),
        colors.hex("#eb6a07", "npm update")
      ));
  } catch (_) {}

  // ─── Instagram login (replaces login.js) ───────────────────────────────────
  require("./bot/login/loginInstagram.js");
})();

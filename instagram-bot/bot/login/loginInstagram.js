...
(async function startInstagramBot() {
  try {
    ...
    // Check credentials — must have either a session export or username+password
    const hasSessionState = !!process.env.IG_SESSION_STATE;
    const hasCredentials = !!(process.env.IG_USERNAME && process.env.IG_PASSWORD);
    if (!hasSessionState && !hasCredentials) {
      log.err("INSTAGRAM", "Instagram credentials required. Set one of:");
      log.err("INSTAGRAM", "  Option 1 (recommended): IG_SESSION_STATE env var (base64 session export)");
      log.err("INSTAGRAM", "  Option 2: IG_USERNAME + IG_PASSWORD env vars");
      log.err("INSTAGRAM", "  See .env.example for details, then copy to .env and restart.");
      process.exit(1);
    }
    ...
    if (hasCredentials) {
      ...
      log.info("INSTAGRAM", "Restoring session from IG_SESSION_STATE...");
    }
    ...
    // ── Expose bot info to global for dashboard ────────────────────────────────
    try {
      const selfInfo = await api.getSelfInfo();
      global.GoatBot.botUsername = selfInfo.vanity || selfInfo.name;
      global.GoatBot.authMethod = api._loginMethod || "instagram-private-api";
    } catch (_) {
      global.GoatBot.botUsername = process.env.IG_USERNAME || "bot";
      global.GoatBot.authMethod = "instagram-private-api";
    }

    log.info("INSTAGRAM", `Logged in successfully ✓  botID: ${global.botID}  @${global.GoatBot.botUsername}`);
    ...
    async function callBackListen(error, event) {
      ...
      // ── Track event in admin stats ─────────────────────────────────────────
      try {
        if (global.adminStats && event) {
          global.adminStats.trackMessage(event);
        }
      } catch (_) {}
